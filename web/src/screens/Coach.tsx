...
import { saudacao } from '../utils/format';
...
const MENSAGEM_POR_TIPO_ERRO: Record<string, string> = {
  rate_limited: 'Você atingiu o limite de mensagens de hoje com o Conselheiro. Volta amanhã!',
  budget_exceeded: 'O Conselheiro está temporariamente indisponível. Tente de novo mais tarde.',
  provider_unavailable: 'O Conselheiro está indisponível no momento. Tente de novo em instantes.',
...
// Nome de exibição pro vendedor é "Conselheiro" (Fatia 7.5A, seção 20) —
// identificador interno continua Coach (rota /coach, tipos, componente,
// especialista de IA) pra evitar refactor/migração desnecessários.
export function Coach() {
  const { sessao } = useAuth();
  const primeiroNome = sessao?.vendedor.nome.split(' ')[0] ?? '';
  ...
  async function carregarTudo() {
    ...
    try {
      ...
      setErroCarregamento('Não foi possível carregar o Conselheiro agora.');
    } finally {
      setCarregando(false);
    }
  }
  ...
  if (carregando) return <LoadingState texto="Carregando o Conselheiro..." />;
    ...
    <div className="mx-auto flex h-full w-full max-w-md flex-col">
      <div className="flex-1 overflow-y-auto p-4 pb-4">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold text-white">Conselheiro</h1>
        </div>

        {!checkinHoje && (
          <Card className="mb-4">
            {/* Saudação por horário + nome (Fatia 7.5A, seção 22) — reaproveita
                o check-in diário já existente, não duplica um "sistema de humor". */}
            <p className="mb-3 text-slate-200">
              {saudacao()}, {primeiroNome}! Como você está chegando pra trabalhar hoje?
            </p>
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 border-t border-slate-800 bg-base p-3 pb-[calc(env(safe-area-inset-bottom)+12px)]">
        <input
          ...
          placeholder="Fala com o Conselheiro..."
          disabled={enviando}
