...
/**
 * Rotas reais do app (Etapa 2A). Até aqui este service emitia `href` para um
 * `/evoluir/*` que NUNCA existiu em `web/src/App.tsx` — 5 dos 6 links levavam
 * a tela em branco (não há rota curinga), justamente na única superfície de
 * "próxima ação" do produto. O `?aba=` existe pra o clique aterrissar na aba
 * certa, não só em qualquer lugar que responda 200 — a Academia não precisa de
 * parâmetro porque lista todas as trilhas numa página só, e o título do item já
 * diz qual delas é.
 */
const ROTAS = {
  academia: '/academia',
  minhaEvolucao: '/universidade?aba=evolucao',
  meuPlano: '/universidade?aba=plano',
  certificacoes: '/universidade?aba=certificacoes',
  revisao: '/universidade/revisao',
  missoes: '/missoes',
} as const;
...
export async function montarParaVoce(vendedorId: string, papel: Papel, limite = 5): Promise<ItemParaVoce[]> {
  ...
  if (papel === 'VENDEDOR' || papel === 'GERENTE') {
    ...
    for (const t of trilhasOnboarding) {
      ...
      if (pendente) itens.push({ tipo: 'ONBOARDING', titulo: `Trilha de integração: ${t.title}`, descricao: 'Trilha obrigatória pra quem está começando.', refId: t.id, href: ROTAS.academia });
    }
  }
  ...
  for (const c of matriz.filter((m) => m.priority === 'HIGH')) {
    itens.push({ tipo: 'GAP_TRACK', titulo: `Reforçar ${c.name}`, descricao: `Sua pontuação está abaixo da meta nesta competência.`, refId: c.competencyId, href: ROTAS.minhaEvolucao });
  }
  ...
  if (revisoesPendentes > 0) {
    itens.push({ tipo: 'REVIEW', titulo: `${revisoesPendentes} revisão(ões) pendente(s)`, descricao: 'Questões que você errou recentemente — vale revisar agora.', refId: 'review', href: ROTAS.revisao });
  }
  ...
  for (const p of pdisAtivos) {
    itens.push({ tipo: 'PDI', titulo: `Continuar seu plano: ${p.competencia.name}`, descricao: 'Você tem um plano de desenvolvimento em andamento.', refId: p.id, href: ROTAS.meuPlano });
  }

  // 5. Missões ativas.
  const missoesAtivas = await prisma.missionAssignment.count({ where: { vendedorId, status: { in: ['ASSIGNED', 'IN_PROGRESS'] } } });
  if (missoesAtivas > 0) {
    itens.push({ tipo: 'MISSION', titulo: `${missoesAtivas} missão(ões) em andamento`, descricao: 'Complete suas missões de hoje.', refId: 'missoes', href: ROTAS.missoes });
  }
  ...
  for (const cert of expirando) {
    itens.push({ tipo: 'CERTIFICATION', titulo: `Certificação expirando: ${cert.definicao.name}`, descricao: 'Renove antes que expire.', refId: cert.id, href: ROTAS.certificacoes });
  }
