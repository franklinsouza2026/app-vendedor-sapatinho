...
### Fatia 7.5B — Admin AI Control Plane + Múltiplos Providers de IA
Registrado a partir da visão trazida na Fatia 7.5A (seção 45-47) — ainda não implementado.
- Admin poderá configurar, testar e trocar entre 3 providers (ANTHROPIC, OPENAI, GEMINI) sem redeploy — apenas UM provider principal ativo por configuração inicial; fallback/AUTO fica fora de escopo até haver política explícita de quando trocar automaticamente.
- Admin vê status, modelo, uso e custo/budget do provider ativo (a `AIBudgetConfig`/`AIUsage` já existentes desde a Fatia 4 são a base — este item é a camada de configuração/gestão por cima, não uma reconstrução).
- Gestão segura de API keys: Admin insere/atualiza a chave; depois de salva, ela NUNCA é mostrada de novo (UI mostra "Configurada ✓" + "Atualizar chave", nunca "mostrar senha"). Chave nunca aparece em bundle de frontend, `localStorage`, logs, JWT, Git, ou em texto puro casual no banco — a fatia que implementar isso escolhe o mecanismo de armazenamento seguro (ex. criptografia com chave de aplicação vinda de env, nunca "criptografia caseira").
- Preparar a navegação do Admin (seções "IA" já reservada na Fatia 7.5A) sem criar telas/config falsas antes da implementação real.

### Fatia 7.5C — CMS + Biblioteca + Academia Administrável + 13 Mandamentos + Quiz Dinâmico
- Transformar a Academia de Vendas (hoje seed estático da Fatia 6) numa Academia administrável pelo Admin: trilhas/aulas/materiais (TEXT/VIDEO/MATERIAL·ATTACHMENT/MIXED) e quizzes criados/editados/publicados/arquivados via painel, com versionamento.
- **13 Mandamentos das Vendas Sapatinho de Luxo** (nome oficial — substitui qualquer referência anterior a "3 Mandamentos" na documentação/requisitos do produto): conteúdo oficial ainda **pendente de fornecimento pelo Admin/usuário** — não inventado nem gerado por IA nesta fatia nem em nenhuma futura sem a fonte real, mesma disciplina já aplicada ao Playbook (Fatia 5, `PLAYBOOK_BASE_DEMONSTRATIVO` vs. `OFICIAL`).
- Quiz dinâmico: banco de questões, seleção dinâmica (perguntas variam entre tentativas), dificuldade, questões situacionais, versionamento de conteúdo/quiz, gabarito e score sempre calculados no backend (nunca confiar no frontend), rastreabilidade da fonte, nenhuma pergunta fora do escopo do conteúdo oficial. IA pode PROPOR perguntas; aprovação humana decide o que entra no banco real.

### Fatia 7.5D — Training Intelligence Platform
- `TrainingOrchestrator` coordenando especialistas lógicos que compartilham a AI Platform já existente (Fatias 4-6) — nunca 9 frameworks de IA independentes: `ResearchAgent`, `CuratorAgent`, `InstructionalDesignerAgent`, `QuizAgent`, `SimulationDesignerAgent`, `SellerTrainingAgent`, `ManagerTrainingAgent`, `GovernanceAgent`, `ContentUpdateAgent`.
- Pesquisa externa (temas: técnicas de vendas, varejo, atendimento, negociação, objeções, fechamento, venda complementar, liderança, gestão de equipe, feedback, motivação, treinamento, organização financeira pessoal, comunicação, inteligência emocional profissional, carreira, tendências comerciais, entre outros úteis à formação) — fornecedor/API externa NÃO escolhido nesta fatia (Fatia 7.5A só registra o requisito e a interface futura).
- Proveniência obrigatória em todo conteúdo pesquisado/gerado: `sourceType`, `sourceUrl`, `sourceTitle`, autor/publisher quando disponível, datas de publicação/descoberta/coleta, resumo, tópico, avaliação de confiança/relevância, versão do conteúdo, status de revisão, quem revisou/aprovou.
- Classificação editorial obrigatória: `OFFICIAL_COMPANY` / `ADMIN_CURATED` / `AI_RESEARCHED` / `AI_GENERATED` — conteúdo `AI_RESEARCHED`/`AI_GENERATED` NUNCA vira política oficial automaticamente.
- Workflow editorial: `DRAFT → RESEARCHED → CURATED → REVIEW_PENDING → APPROVED → PUBLISHED → ARCHIVED` (ou equivalente); Admin mantém autoridade final; auto-publicação de conteúdo de baixo risco só mediante configuração explícita futura — o padrão é sempre aprovação humana.

### Fatia 7.5E — Universidade de Vendedores + Universidade de Gerentes + Matriz de Competências + Trilhas Inteligentes
- **Universidade de Vendedores** — categorias futuras (conteúdo real pendente, nenhuma aula inventada agora): Fundamentos de Vendas, 13 Mandamentos, Abordagem, Sondagem, Necessidades, Demonstração, Argumentação, Venda Complementar, PA, Ticket, Objeções, Negociação, Fechamento, Pós-venda, Fidelização, WhatsApp, Carteira de Clientes, Produto, Experiência do Cliente, Organização, Produtividade, Metas, Comunicação, Ética, Carreira, Organização Financeira Pessoal.
- **Universidade de Gerentes** — categorias futuras: Liderança, Gestão de Equipe, Metas, Indicadores, Reunião Diária, Feedback, 1:1, Desenvolvimento, Coaching de Campo, Baixa Performance, Reconhecimento, Motivação, Conflitos, Onboarding, Treinamento, Escalas, Produtividade, Campanhas, Delegação, Comunicação, Tomada de Decisão, Formação de Líderes, Organização do Gerente, Gestão Financeira/indicadores pertinentes, Cultura, Ética.
- **Matriz de Competências**: score derivado de evidências estruturadas (nunca opinião arbitrária do LLM) — vendedor (Abordagem/Sondagem/Argumentação/Objeções/Venda Complementar/Fechamento/Conhecimento dos Mandamentos), gerente (Liderança/Feedback/Gestão de Meta/Análise/Desenvolvimento/Comunicação/Gestão de Conflito).
- **Trilhas Inteligentes** — princípio central do produto, ciclo: DETECTAR LACUNA → RECOMENDAR CONTEÚDO → APRENDER → QUIZ → TREINAR → SIMULAR → MISSÃO → MEDIR NOVAMENTE.
- **Certificações**: por trilha, com recertificação/validade/histórico — especialmente pros 13 Mandamentos das Vendas Sapatinho de Luxo.
